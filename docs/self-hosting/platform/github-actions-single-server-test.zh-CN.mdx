...
description: >-
  使用 GitHub Actions、Docker Compose 和 SSH 将 LobeHub
...
- `DOCKER_REGISTRY_IMAGE` 填服务器本地要使用的镜像仓库名，例如 `mylob/test-deploy`
...
1. Checkout 目标代码
2. 在 GitHub Actions 中构建测试镜像，并打包为镜像归档文件
3. 通过 SSH 将镜像归档、`docker-compose.yml` 和 `.env` 上传到服务器
4. 在服务器执行 `docker load && docker compose up -d`
5. 检查首页是否可访问

## 回滚

若某次测试部署有问题，可以重新手动触发工作流，并在 `image_tag` 中填写服务器上已经存在的一个可用镜像 Tag。

例如：

`test-canary-abc1234`

这会跳过构建和镜像上传，直接将服务器切回指定镜像版本。

注意：回滚时请让 `ref` 指向构建该镜像的同一提交。最稳妥的做法是直接填写对应的 Commit SHA，避免旧镜像与不匹配的部署文件混用。如果这个 `image_tag` 从未被加载到当前服务器，回滚会直接失败。
