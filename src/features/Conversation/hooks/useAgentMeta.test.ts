...
describe('useAgentMeta', () => {
  ...
  it('should return 北测数字 AI title for builtin inbox agent, preserving avatar from backend', () => {
    ...
    // Should override title with 北测数字 AI, but preserve avatar from backend
    expect(result.current.avatar).toBe('/icons/icon-lobe.png');
    expect(result.current.title).toBe('北测数字 AI');
    // Should preserve other properties
    expect(result.current.description).toBe('Inbox description');
  });

  it('should return 北测数字 AI title for page agent (builtin), preserving avatar from backend', () => {
